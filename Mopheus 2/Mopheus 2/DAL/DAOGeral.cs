...
namespace DAL
{
    public class DAOGeral : Conexao
    {
        public void executaNormal(string query)
        {
            try
            {
                open();
            ...
            catch(Exception ex)
            {
                ...
                try
                {
                    ...
                    close();
                    throw new System.InvalidOperationException("Ocorreu erro no processo!");
                }
                catch(Exception ex2)
                {
        ...
        public DataTable executaComRetorno(string query)
        {
            try
            {
                open();
                DataTable dtretorno = executeGetResult(query);
                close();
                return dtretorno;
            }
            catch (Exception ex)
