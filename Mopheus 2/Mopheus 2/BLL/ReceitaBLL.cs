...
namespace BLL
{
    public class ReceitaBLL : BaseBLL<Receita>
    {
        ...
        public override Receita ConvertToClass(DataRow row)
        {
            ...
            obj.dispositivo= row["dispositivo"].ToString();
            obj.endereco = Convert.ToInt32(row["endereco"].ToString());
            obj.repeticao = row["repeticao"].ToString();
            obj.vezes = Convert.ToInt32(row["vezes"].ToString());
            obj.intervalo = row["intervalo"].ToString();
